...
Multi-Scale CNN LOSO for facial EMG / silent speech command recognition.
...
推荐运行：
    python eval_loso_mscnn.py --norm per_window --epochs 40 --batch_size 128 --out_dir .\results_loso_mscnn_new

如果某一类召回率很低，可尝试：
    python eval_loso_mscnn.py --norm per_window --class_weight --epochs 40 --batch_size 128 --out_dir .\results_loso_mscnn_new
"""
...
def normalize_per_window(X, eps=1e-6):
    """
    每个窗口、每个通道单独 z-score。
    和你原 CNN 的 per_window_norm=True 接近。
    X: [N,C,T]
    """
...
def normalize_subject_p95(X, eps=1e-6):
    """
    近似 MVC/p95，每组数据每通道按 95% 幅值分位数归一化。
    这里只作对照，不建议作为主要结论。
    """
...
class ConvBNAct(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, dropout=0.0):
        super().__init__()
...
class MultiScaleCNN(nn.Module):
    """
    Multi-Scale CNN for sEMG windows.

    参考多流/多尺度 sEMG CNN 思想：
    - 小卷积核 kernel=3：局部快速变化、跳变
    - 中卷积核 kernel=7：中尺度波形形态
    - 大卷积核 kernel=15：较长动作趋势
    ...
    """
    def __init__(self, in_ch=5, num_classes=3, base_ch=32, dropout=0.35):
        super().__init__()

        self.branch3 = nn.Sequential(
            ConvBNAct(in_ch, base_ch, kernel_size=3, dropout=0.0),
            ConvBNAct(base_ch, base_ch, kernel_size=3, dropout=0.0),
            nn.MaxPool1d(kernel_size=2),
        )

        self.branch7 = nn.Sequential(
            ConvBNAct(in_ch, base_ch, kernel_size=7, dropout=0.0),
            ConvBNAct(base_ch, base_ch, kernel_size=7, dropout=0.0),
            nn.MaxPool1d(kernel_size=2),
        )

        self.branch15 = nn.Sequential(
            ConvBNAct(in_ch, base_ch, kernel_size=15, dropout=0.0),
            ConvBNAct(base_ch, base_ch, kernel_size=15, dropout=0.0),
            nn.MaxPool1d(kernel_size=2),
        )
        ...
        self.fusion = nn.Sequential(
            ConvBNAct(merge_ch, 96, kernel_size=5, dropout=dropout),
            ConvBNAct(96, 128, kernel_size=3, dropout=dropout),
    ...
    def forward(self, x):
        ...
        # 三个分支都经过 MaxPool1d(2)，时间长度一致。
        y = torch.cat([y3, y7, y15], dim=1)
        y = self.fusion(y)
...
def run_loso(args):
    ...
    exp_name = (
        f"mscnn_norm-{args.norm}_calib-{args.calib_ratio:.2f}"
        f"_T{time_len}_base{args.base_ch}_drop{args.dropout}"
    )
    ...
    for test_subj in subject_dirs.keys():
        ...
        print("\n" + "=" * 80)
        print(f"LOSO MS-CNN test={test_subj}; train={'+'.join(train_subjs)}; norm={args.norm}; calib={args.calib_ratio}")
        print("=" * 80)
        ...
        plot_confusion_matrix(cm, run_dir / "confusion_matrix.png", f"MS-CNN test={test_subj}")
        plot_curve(hist["train_loss"], hist["val_loss"], run_dir / "loss_curve.png", "Loss", f"Loss MS-CNN test={test_subj}")
        plot_curve(hist["train_acc"], hist["val_acc"], run_dir / "accuracy_curve.png", "Accuracy", f"Accuracy MS-CNN test={test_subj}")
    ...
    csv_path = out_root / "loso_mscnn_results.csv"
    ...
    plot_confusion_matrix(cm_all, out_root / "confusion_matrix_overall.png", "MS-CNN overall")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Multi-Scale CNN LOSO for facial EMG windows.")
    ...
    parser.add_argument("--out_dir", type=str, default=r".\results_loso_mscnn_new")
